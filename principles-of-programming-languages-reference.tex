...
% TODO: when everything has been written, space correctly the content by breaking the columns via the \switchcolumn command
...
\end{lstlisting}
  ...
  All the operators are in the form \iracket{(<operator> <operand> ...)} \textit{(prefix notation).}
...
\end{lstlisting}
  ...
  \begin{itemize}
    ...
    \item car and cdr can be composed \textit{(\iracket{cdadadr}, \iracket{caaar})}
...
(vector-ref '#(1 2 3) 0) ; => 1
  ...
  \begin{itemize}
    ...
    \item test if an element is in the set: \iracket{set-member?} \textit{(returns a boolean)}
...
(set-member? (set 1 2 3) 2) ; => #t
  ...
  \begin{itemize}
    ...
    \item test if a key is in the hash: \iracket{hash-has-key?} \textit{(returns a boolean)}
  \end{itemize}
